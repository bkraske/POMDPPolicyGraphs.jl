...
"""
    belief_value_recursive(pomdp::POMDP{S,A}, pol::Policy, b::DiscreteBelief, depth::Int;rewardfunction=VecReward())
...
function belief_value_recursive(pomdp::POMDP{S,A}, pol::Policy, b::DiscreteBelief, depth::Int;rewardfunction=VecReward()) where {S,A} #TYLER
    d = 1
    ...
    if d<depth
        ...
        for o in axes(obs,2)
            ...
            if po > 0. && !isterminalbelief(s_pomdp,bp)
                bp.nzval ./= po
                value += discount(s_pomdp)*po*belief_value_recursive(pomdp, s_pomdp, pol, bp, depth, d)
            end    
        end
    end
    return value
